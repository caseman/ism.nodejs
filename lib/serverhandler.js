    ...
    server.game(msg.game, function(err, game) {
        if (err) {
            ...
            if (nation) {
                ...
                client.send({says:'game', game:game.info, re:msg.uid});
                if (game.started()) {
                    var people = nation.people.map(function(uid) {return game.objects[uid]});
                    client.send({says:'update', objects:people});
                }
            } else {
