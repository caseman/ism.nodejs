...
handlers.movePerson = function(server, client, msg) {
    server.requirePlayingClient(client);
    if (~client.nation.people.indexOf(msg.person)) {
        var person = client.game.objects[msg.person];
        object.eventHandler('move', person)(client.game, msg.from, msg.to);
        return true;
    }
}

handlers.endTurn = function(server, client, msg) {
    server.requirePlayingClient(client);
    ...
    for (var uid in client.game.nations) {
        ...
        if (nation.player.type && nation.player.turnCompleted < client.game.info.turnNumber) {
            client.send({says:'endTurn', wait:true, re:msg.uid});
            return true;
        }
    }
    client.send({says:'endTurn', wait:false, re:msg.uid});
