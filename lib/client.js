...
function Client(serverInfo) {
    ...
    this.movePerson = function movePerson(uid, dx, dy) {
        var person = this.gameState.people[uid]
          , toLocation = [person.location[0] + dx, person.location[1] + dy];
        this.send({says:'movePerson', person:uid, from:person.location, to:toLocation});
    }
    ...
    this.on('msg update', function(msg) {
        ...
        if (msg.game && (!game || game.info.uid != msg.game.uid)) {
            ...
            if (msg.nation) {
                ...
                client.emit('updateNation', msg.nation, oldNation, game);
            }
            if (msg.people) {
                for (var uid in msg.people) {
                    ...
                    client.emit('updatePerson', person);
                }
            }
        }
    });
