...
Map.prototype.tileFuncs = function(params) {
    ...
    var neighborTiles = function(tile, tiles, xOffset) {
        ...
        var left = tiles[(x - 1 + width) % width],
    ...
    var channelRiver = function(start, end, tiles) {
        var river = aStar({
          ...
          , distance: function(a, b) {
                var dy = a.y - b.y
                  , dx = a.x - b.x
                  , dist = dx*dx + dy*dy; // manhattan
                // slightly favor straight over diagonal flows
                return b.isLand ? (dist + 1) / b.riverFlow : 0;
            }
        ...
        function biome(tile) {
            ...
            if (tile.elevation + tile.elevation * tile.ruggedness > freezeLevel * params.iceThreshold || riverFlow && tile.elevation * tile.elevation > tile.temperature) {
                ...
                if (tile.ruggedness > tile.temperature * tile.temperature) {
                    ...
                    if ((tile.rainfall > 5 || riverFlow > 1 && tile.rainfall - tile.ruggedness > tile.temperature) && Math.random() < params.marshDensity) {
                        tile.biome = 'marsh';
                    } else if (tile.temperature > 0.8 && tile.rainfall + riverFlow > 3 && tile.ruggedness > 0.1) {
                        tile.biome = 'jungle';
