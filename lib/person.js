...
object.define('person', 
    {
      ...
      , sees: function(game, obj) {
            var objCopy = object.clientCopy(obj)
              , tileSeen = this.tilesSeen[obj.location];
            if (tileSeen && tileSeen.move == this.moveNumber) {
                tileSeen.objects.push(objCopy);
                this.happenings.push({what:'saw', subject:objCopy, turn:game.info.turnNumber});
                game.saveObject(this);
            }
        }
    }
  , ['hp', 'maxHp', 'stamina', 'maxStamina', 'bonusTraits', 
     'tilesSeen', 'items', 'equipment', 'skills']
);
...
function see(person, game) {
    ...
    sightFrom(game, person.location, person.sight, function(tile, elev, tileKey) {
        ...
        objects = objects.map(function(obj) {return object.clientCopy(obj)});
        person.tilesSeen[tileKey] = {
          ...
          , objects: objects
          , turn: turn
          , move: person.moveNumber
        };
        if (objects.length) {
            var newHappenings = objects.map(function(obj) {
                return {what:'saw', subject:obj, turn:turn};
            });
            person.happenings = person.happenings.concat(newHappenings);
        }
    });
}
exports.see = see;
