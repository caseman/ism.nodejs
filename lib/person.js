...
function personChanged(person, game) {
    var nation = game.nations[person.nationUid];
    if (nation) nation.emit('personChanged', person);
}
...
object.define('person', 
    {
      ...
      , sees: function(game, tile, looking) {
            if (looking || this.tilesSeen[tile.key]) {
                ...
                if (nation) {
                    nation.rememberTile(tile);
                    if (!looking) nation.emit('personChanged', person);
                }
