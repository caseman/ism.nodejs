...
var Game = Ctor(function() {
    this.init = function(db, uid, properties) {
        ...
        this.tileKey = function(x, y) {
            return key('g', uid, 'tile', pad(x), pad(y));
        }
    }

    /*
     * return the current turn number
     * will be undefined if game is not started
     */
    ...
    this.nextTurn = function() {
        this.turn.time = timestamp();
        this.turn.number = this.turn.number ? this.turn.number + 1 : 1;
        this.db.put(key('g', this, 'turn'), this.turn);
        return this.turn.number;
    }

    this.uidsAtLocation = function(location) {
        var col = this.objectIndex[location[0]];
    ...
    this.saveObject = function(object, cb, batch) {
        ...
        batch.put(key('g', this, 'obj'), object, cb);
    }
    ...
    this.placeObject = function(object, location) {
        ...
        var col = this.objectIndex[location[0]];
        if (!col) col = this.objectIndex[location[0]] = [];
     ...
     */
    this.createObject = function(type, location) {
        ...
        if (location) this.placeObject(object, location);
        return object;
    }
 ...
 */
exports.createGame = function(db, map, startMonth, turnsPerMonth, cb) {
    var uid = genUid()
      , properties = {
            info: {
                uid: uid
    ...
    for (var name in properties) {
        batch.put(key('g', game, name), properties[name]);
    }
    game.tiles = [];
 ...
 */
exports.loadGame = function(db, uid, loadCb) {

    var get = function(subKey) {
        return function(cb) {
            db.get(key('g', uid, subKey), cb);
        }
    }

    var load = function(subKey, dest, cb, dataCb) {
        var startKey = key('g', uid, subKey)
          , ended = false;

        db.createValueStream({start: startKey, end: startKey + '~'})
            .on('data', dataCb)
            .on('end', function() {
                ended = true;
                cb(null, dest);
            ...
            .on('close', function() {
                if (!ended) cb(new Error("Db stream closed prematurely loading game " + uid));
            })
            .on('error', cb);
    }
    ...
    async.series(
        {
          ...
          , tiles: function(cb) {
                var tiles = [];
                load('tile', tiles, cb, function(tile) {
          ...
          , objects: function(cb) {
                var objects = {};
                load('obj', objects, cb, function(obj) {
