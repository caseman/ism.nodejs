...
var Game = Ctor(events.EventEmitter, function() {
    ...
    this.placeObject = function(obj, location) {
        ...
        obj.location = [x, y];
        var onlookers = this.sightIndex[obj.location];
        if (onlookers) object.sendEvent('sees', onlookers, this, obj);
        this.saveObject(obj);
        return obj;
    }

    /*
     * Mark a location as seen by something this turn
     * Return a list of objects at this location
     */
    this.iSee = function(person, location) {
        var x = location.x || location[0] || 0
          , y = location.y || location[1] || 0
          , tileKey = x + ',' + y
        ...
        return this.objectsAtLocation(location);
    }
