...
exports.Server = function Server(db) {
    ...
    this.start = function start(port, host, _cb) {
        ...
        var infoFile = fs.createWriteStream(serverInfoPath(this.db.location));
        infoFile.write(JSON.stringify(info, null, 2), _cb);
...
function RemoteClient(conn) {
    ...
    this.joinGame = function(game, nation) {
        ...
        if (game.started()) {
            this.sendUpdate({game:game.info, nation:nation, people:people});
        }
    }
...
exports.info = function(dbPath) {
    ...
    if (fs.existsSync(infoPath)) {
        try {
            ...
            // ignore require and process.kill errors
        }
    }
}
