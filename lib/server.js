...
function Client(conn) {
    ...
    this.sendUpdate = function(uids) {
        var msg = {says:'update', game:this.game.info};
        if (!uids) {
            var uids = Object.keys(this.updates);
            this.updates = {};
            this.updateTask = null;
        }
        var gameObjects = this.game.objects;
        msg.objects = uids.map(function(uid) {
            return object.clientCopy(gameObjects[uid]);
        });
        this.send(msg);
    }

    this.objectChanged = function(obj) {
        if (obj.nationUid == this.nation.uid) {
            this.updates[obj.uid] = obj;
            if (!this.updateTask) {
                var client = this;
    ...
    var objectChanged = this.objectChanged.bind(this);

    this.joinGame = function(game, nation) {
        this.game = game;
        this.nation = nation;
        game.on('objectChanged', objectChanged);
        game.db.put(key('player', 'client', this.cid), 
            {game:game.uid, nation:nation.uid}
        );
        this.sendUpdate(nation.people);
    }

    this.closed = function() {
        this.conn = null;
        if (this.game) this.game.removeListener('objectChanged', objectChanged);
