...
exports.Server = function Server(db) {
    ...
    this.sockjsServer.on('connection', function(conn) {
        ...
        conn.on('data', function(message) {
            ...
            try {
                ...
                switch (err.message) {
                    case 'timeout':
                        log.info('Operation timed out for', parsedMsg, 'from', client);
                        server.sendError(conn, 'timeout', parsedMsg);
                        return;
                    case 'unknownConnection':
                        log.info('Operation not permitted from unknown connection from', client);
                        server.sendError(conn, 'unknownConnection', parsedMsg);
                        return;
                    default:
    ...
    this.cidForConn = function(conn) {
        return conn.ISM_cid;
    }

    this.requireCid = function(conn) {
        var cid = this.cidForConn(conn);
        if (!cid) throw new Error('unknownConnection');
        return cid;
    }
}
