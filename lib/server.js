...
exports.Server = function Server(db) {
    ...
    var sockjsOptions = {
        log: function(severity, line) {
            if (log[severity]) log[severity](line);
        }
    };
    ...
    this.sockjsServer.on('connection', function(conn) {
        ...
        conn.on('data', function(msgStr) {
            ...
            try {
                handled = msg.says && msg.uid && handler.handle(server, client, msg);
            } catch (err) {
                switch (err.message) {
                    case 'timeout':
...
function RemoteClient(conn) {
    ...
    this.unitChanged = function(unit) {
        if (!this.nextUpdate.units) this.nextUpdate.units = {};
        this.nextUpdate.units[unit.uid] = object.clientCopy(unit, this);
        this.scheduleUpdate();
    }
    var unitChanged = this.unitChanged.bind(this);

    this.nationChanged = function(nationInfo) {
        this.nextUpdate.nation = nationInfo;
        this.scheduleUpdate();
    }
    var nationChanged = this.nationChanged.bind(this);

    this.joinGame = function(game, nation) {
        this.game = game;
        this.nation = nation;
        game.on('infoChanged', gameInfoChanged);
        nation.on('unitChanged', unitChanged);
        nation.on('nationChanged', nationChanged);
        game.db.put(key('player', 'client', this.cid),
            {game:game.uid, nation:nation.uid}
        );
        var units = {}
          , client = this;
        nation.units.forEach(function(uid) {
            units[uid] = object.clientCopy(uid, client);
        });
        this.nextUpdate.game = game.info
        this.nextUpdate.nation = nation
        this.nextUpdate.units = units
    ...
    this.closed = function() {
        ...
        if (this.nation) {
            this.nation.removeListener('unitChanged', unitChanged);
            this.nation.removeListener('nationChanged', nationChanged);
        }
